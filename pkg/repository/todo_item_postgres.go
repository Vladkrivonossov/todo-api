...
func (r *TodoItemPostgres) GetAll(userId, listId int) ([]todo.TodoItem, error) {
  ...
  query := fmt.Sprintf("SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li on li.item_id = ti.id INNER JOIN %s ul on ul.list_id = li.list_id WHERE li.list_id = $1 AND ul.user_id = $2", todoItemsTable, todoListsTable, usersListsTable)
...
func (r *TodoItemPostgres) GetById(userId, itemId int) (todo.TodoItem, error) {
  ...
  query := fmt.Sprintf("SELECT ti.id, ti.title, ti.description, ti.done FROM %s ti INNER JOIN %s li on li.item_id = ti.id INNER JOIN %s ul on ul.list_id = li.list_id WHERE ti.id = $1 AND ul.user_id = $2", todoItemsTable, todoListsTable, usersListsTable)
